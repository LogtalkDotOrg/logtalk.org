...
* FIXED: Bug when handling entity operators defined in an included file where
they would be visible outside the entity.
...
* ADDED: New `sarif` tool for generating SARIF reports from diagnostics
generated by other developer tools.
...
* ADDED: New `linter_reporter` tool support for generating diagnostics that
can be used by tools such as `sarif`.

* ADDED: Support to the `dead_code_scanner`, `lgtdoc`, and `lgtunit` tools for
generating diagnostics that can be used by tools such as `sarif`.

* ADDED: Support for caller diagrams to the `diagrams` tool.

* ADDED: Implementations of the Lines, Maintainability Index, Lack of Cohesion
...
* ADDED: Support to the `diagrams` tool `lgt2svg.sh` and `lgt2svg.ps1` scripts
to export diagrams in formats other than SVG.
...
* UPDATED: The `tutor` tool to provide a `tutor_explanations` category with
the public `explain//1` non-terminal that can be used independently by other
tools.
...
* FIXED: Bug in the `packs` tool `registries::defined/4` predicate where local
registries that are git repos would not be recognized as such.
...
* ADDED: New `cuid2`, `ksuid`, `nanoid`, and `snowflakeid` libraries
implementing, respectively, the Cuid2, KSUID, NanoID, and Snowflake ID
specs.
...
* ADDED: Predicate `help::man/1` tool for browsing scripts documentation.

* UPDATED: The `help` tool to allow selecting between browsing the HTML files
generated by Sphinx for web browsing or the XHTML files generated by Sphinx
for the ePub version of the documentation.

* UPDATED: The `logtalk_tester.sh`, `logtalk_tester.ps1`, `logtalk_doclet.sh`,
and `logtalk_doclet.ps1` scripts to also print the runtime.
...
* IMPROVED: The `logtalk_doclet.sh` and `logtalk_tester.sh` scripts to print
only three decimal places for test set runtime and clause coverage percentage.
...
* FIXED: The `logtalk_doclet.sh` and `logtalk_tester.sh` scripts to also
check for the `gnutimeout` alternative name for the GNU `coreutils` command
`timeout`. Thanks to Andrew Davison for the bug report.
...
* FIXED: The `debugger` tool missing tokenization for the message
`matching_conditional_spy_points_removed`.
...
as Jupyter notebooks. Experimental. See the `examples/NOTES.md` file for the
setup details.
...
* FIXED: VSCode support for finding predicate references when the call is
...
* FIXED: Bug in the `diagrams` tool where file loading diagrams would show
duplicated "includes" edges.
...
* IMPROVED: The Handbook documentation on performance, adding a section on
circular references.
...
`call/1` control construct and `call/N` built-in predicates with JIProlog.
...
* IMPROVED: Detect and report when a `uses/2` directive references the object
that contains it.
...
`clause/2`, `nl/0-1`, `op/3`, `retractall/1`, and `term_variables/2`
built-in predicates.
...
* ADDED: Compiler support for using backend proprietary directives in calls
...
* UPDATED: The LVM adapter file to support encryption of the intermediate
Prolog files generated by the compiler by setting the `prolog_compiler` flag
to `[encrypt(true)]` and the `clean` flag to `off`.

* ADDED: SWI-Prolog adapter file initial support for using tabling directives
and predicates in dynamically created objects.
...
* ADDED: New `diagrams` tool `predicate_url_target_format/1` option (default
is `sphinx`) to allow generating correct documentation URL predicate targets.
...
* FIXED: Tests for the `ulid` library to account for some backends providing
low time accuracy.
...
compilation messages for the intermediate Prolog files generated by the Logtalk
compiler.
...
`atom_concat/3`, `number_chars/2`, `number_codes/2`, `stream_property/2`,
and `sub_atom/5` predicates. Some of them taken or derived from the WG17
test suite.
...
* ADDED: Adapter file for Trealla Prolog. Experimental. With the help of
Andrew Davison.
...
* ADDED: Additional test for the `debugger::trace/0` predicate.

* IMPROVED: Advice given by the `tutor` tool for warnings generated by compile
time references to unknown entities.
...
* CHANGED: The tests for the `asserta/1` and `assertz/1` Prolog standard
...
license change is kindly sponsored by Kyndi Inc. 
